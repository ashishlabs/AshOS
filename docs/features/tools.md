# Feature: Tools (Shell / Git / Filesystem / Web Fetch)
...
world, instead of just producing text. Four ship by default: **Shell**
(run any command), **Git** (clone/commit/branch/merge/status/diff/log/push),
**Fs** (read/write/list files), and **Web Fetch** (fetch a URL and
extract its readable text — used automatically by the Inbox's AI
summarizer, `docs/inbox.md`). Every dangerous action (shell commands,
...
- **Fs** and **Web Fetch** have no permission-prompt gate — Fs's actions
  are treated as non-destructive by design; Web Fetch is read-only and
  restricted to public http(s) addresses (see below), so neither routes
