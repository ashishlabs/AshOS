...
- Dashboard (Vite + React + Tailwind v4 + shadcn/ui-style components) with status, plan, workflow, memory, logs, chat, Evolution, Innovation, and Trending tabs, a responsive sidebar nav (drawer on mobile), and light/dark theming. Providers/Agents/Tools don't have their own tabs (dropped as unused UI surface) — all three are still fully reachable via `ash provider`/`ash status` and `GET /providers`/`/agents`/`/tools`.
