...
| `agents/` | `Agent` interface plus Code, Research, Git, Testing, Generic, GitHub Trending, and Reflection agents, routed by capability tag. `reflection.ts` (pure Outcome Memory/Inbox/Knowledge Graph summarization, unit tested independently) + `reflection-agent.ts` (`ReflectionAgent`, capability `reflection`) generate a daily/weekly/monthly review narrative — same shape as Innovation's `DailyBriefGenerator` — reused for `ash reflect` / `GET /reflect`. |
...
| `search/` | `HybridSearch` (`AshOS.search`) — merges and ranks results from `MemoryManager`, the general `KnowledgeGraph`, and `InboxManager` into one "find everything about X" list. Owns no storage of its own; every slice queries its own store's existing lookup. `ash search` and `/search`. See `docs/search.md`. |
