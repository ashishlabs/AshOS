...
| `tools/` | `Tool` interface plus shell, git, filesystem, and web-fetch tools, each declaring capabilities/requirements/permissions and a health check. `WebFetchTool` (capability `web-fetch`) fetches a URL and extracts its readable text — http(s) only, private/loopback/link-local addresses blocked after DNS resolution, no redirects followed, timeout + size caps — used by the Inbox's AI summarizer (`docs/inbox.md`) and available to any agent via `context.tools.get("web-fetch")`. `application/pdf` responses (or a `.pdf`-suffixed URL with a generic/absent content-type) are extracted to real text via `pdfjs-dist`, and `image/*` responses are OCR'd via `tesseract.js` (language data bundled locally, no CDN fetch), instead of being rejected as binary. |
| `memory/` | `MemoryManager` unifying short-term (in-process, TTL), session (in-process), project (`.ashos/memory/project.db`) and global (`~/.ashos/memory/global.db`) scopes. project/global persist to a real embedded SQLite database via Node's built-in `node:sqlite` (no native dependency) instead of a whole-file JSON array — a `remember()` call is a single indexed write, and a tag-filtered `query()` (what every subsystem's `list()` actually calls) is an indexed join, not a full scan. An older `project.json`/`global.json` is imported once on first open so upgrading never loses data. Backed by a brute-force cosine-similarity `VectorStore` for semantic recall, rehydrated from the database on construction so it survives process restarts. Every agent task's attempt is auto-recorded here as a `TaskOutcome` (`agents/outcome.ts`) via `BaseAgent.execute()`, with no opt-in needed — see `docs/outcome-memory.md`. |
...
| `workspace/` | Project Workspaces — a real, persisted `Project`/`ProjectTask`/`Milestone` data model, distinct from the label-only `project` Knowledge Graph node `BaseAgent` auto-creates per working directory. Same no-new-persistence-engine convention: `WorkspaceManager`'s records are `MemoryManager` project-scope records tagged `workspace-project`/`workspace-task`/`workspace-milestone`. Tasks/milestones get new `todo`/`milestone` Knowledge Graph node kinds (distinct from the ephemeral `task` kind `BaseAgent` uses for agent-run records) linked to their project via `part-of` edges; `WorkspaceManager.progress()` computes percent-complete on the fly from task statuses. `ash project create/list/show/archive/progress`, `ash project task add/list/status`, `ash project milestone add/list/status`, and `/workspace/*`. See `docs/project-workspaces.md`. |
| `learning/` | Learning Hub — tracked courses/books/videos/articles, plus flashcards reviewed via the SuperMemo-2 (SM-2) spaced repetition algorithm (`srs.ts`, a pure, independently unit-tested function) — the one Second Brain pillar that's genuinely new domain logic rather than a recombination of an existing pattern. Still reuses the same no-new-persistence-engine convention: `LearningManager`'s records are `MemoryManager` project-scope records tagged `learning-resource`/`learning-flashcard`. Resources get a new `learning-resource` Knowledge Graph node kind; flashcards deliberately get none (a deck can hold hundreds of cards — a node per card would add graph noise with no meaningful new connections). `ash learn resource add/list/show/status`, `ash learn card add/list/show/review`, and `/learning/*`. See `docs/learning-hub.md`. |
| `search/` | `HybridSearch` (`AshOS.search`) — merges and ranks results from `MemoryManager`, the general `KnowledgeGraph`, `InboxManager`, `VaultManager`, `WorkspaceManager`, and `LearningManager` into one "find everything about X" list. Owns no storage of its own; every slice queries its own store's existing lookup. `ash search` and `/search`. `agents/ask-agent.ts` (`AskAgent`, capability `ask`/`qa`) is the synthesis layer `HybridSearch` deliberately doesn't have: it runs the same search (semantic by default, since a natural-language question rarely shares literal words with the record that answers it) and has the active provider write a cited answer over the top-ranked excerpts, so "what did I capture about X" gets a written answer with `[n]` citations back to the source record instead of a ranked hit list. `ash ask <question...>` and `POST /ask`. See `docs/search.md`. |
...
| `cli/` | `ash` command (`init`, `doctor`, `status`, `provider`, `plugin`, `plan`, `run`, `chat`, `memory`, `logs`, `innovation`, `codebase`, `graph`, `inbox`, `vault`, `project`, `learn`, `reflect`, `search`). |
| `api/` | Express REST API exposing `/chat`, `/plan`, `/execute`, `/workflow`, `/agents`, `/providers`, `/tools`, `/tasks`, `/memory`, `/logs`, `/health`, `/innovation/*`, `/codebase/*`, `/graph*`, `/inbox*`, `/vault*`, `/workspace/*`, `/learning/*`, `/reflect`, `/search`. |
